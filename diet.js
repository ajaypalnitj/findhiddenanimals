import { ANIMALS, ANIMAL_DIETS } from './animals/config.js';

class DietGame {
    constructor() {
        this.score = 0;
        this.highScore = parseInt(localStorage.getItem('dietHighScore')) || 0;
        this.timeLeft = 30;
        this.timer = null;
        this.cards = [];
        this.selectedCards = [];
    ...
    shareOnTwitter() {
        const text = `I matched ${this.matchedPairs} animal diets and scored ${this.score} points in Diet Match! Can you beat my score?`;
    ...
    copyLink() {
        const text = `I matched ${this.matchedPairs} animal diets and scored ${this.score} points in Diet Match! Try to beat my score at:`;
    ...
    createCards() {
        ...
        animals.forEach(([key, animal]) => {
            ...
            const diet = ANIMAL_DIETS[key];
            cardPairs.push({
                type: 'diet',
                content: `<div class="diet-card">
                    <div class="diet-icon">${diet.icon}</div>
                    <div class="diet-type">${diet.type}</div>
                    <div class="diet-foods">${diet.foods.join(', ')}</div>
                </div>`,
                match: animal.name
            });
        });
    ...
    endGame(won) {
        ...
        if (this.score > this.highScore) {
            ...
            localStorage.setItem('dietHighScore', this.highScore);
        }
        ...
        const endMessage = won 
            ? `Congratulations! You matched all the animals with their diets!<br>Score: ${this.score}`
